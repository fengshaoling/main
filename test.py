...
#这段代码的主要作用是进行目标检测模型的测试。具体来说，它评估模型在验证或测试数据集上的性能，
# 生成检测结果并计算各种性能指标，如精度（Precision）、召回率（Recall）和平均精度（mAP）。以下是代码的主要步骤和功能：
def test(data,
         weights=None,
         batch_size=16,
         imgsz=640,
    ...
    # 创建数据加载器,创建一个数据加载器，用于加载验证或测试集图像
    if not training:
